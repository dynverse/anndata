...
For example, let's download a 10x dataset from the 10x genomics website.
```{r}
...
h5tmp <- tempfile(fileext = ".h5")
download.file(url, destfile = h5tmp)
```

You can transform it to an anndata object with scanpy as follows:
```{r}
library(anndata)
library(reticulate)

sc <- import("scanpy")
ad <- sc$read_10x_h5(h5tmp)
ad
```
