...
The preferred backends (`InMemoryAnnData`, `HDF5AnnData`) require no
Python. For users who already have Python `anndata` installed, the
`ReticulateAnnData` backend offers a low-friction starting point; see
...
The [`AnnData()`](https://anndata.dynverse.org/reference/AnnData.md)
constructor is identical in both packages. The only difference is that
`anndataR` returns an `InMemoryAnnData` (a pure-R object) whereas
...
The function signatures are identical. `anndataR` reads natively without
Python.
...
`anndataR`’s `ReticulateAnnData` backend wraps a Python
`anndata.AnnData` object via reticulate, implementing the same
`AbstractAnnData` interface as the native backends. This is the closest
equivalent to the old `anndata` R package: you can call Python tools
(e.g. `scanpy`) on the object while also using all `anndataR` slot
accessors from R.
