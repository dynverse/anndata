...
test_that("backed indexing", {
  ...
  expect_equal(csc_mem$X, csc_disk$X) # extra test added by me
  expect_equal(as.matrix(csc_mem$X), dense_disk$X) # extra test added by me
  # expect_equal(csc_mem[obs_idx, var_idx]$X, csr_disk[obs_idx, var_idx]$X)
  expect_equal(csc_mem[obs_idx, var_idx]$X, csc_disk[obs_idx, var_idx]$X)
  expect_equal(as.matrix(csc_mem[, var_idx]$X), dense_disk[, var_idx]$X)
  expect_equal(as.matrix(csc_mem[obs_idx, ]$X), dense_disk[obs_idx, ]$X) # why does dense disk only allow one idx?
})
#
